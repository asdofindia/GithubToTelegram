...
DELTATIME=330   #required to adjust the time zone. India is +0530, so 330 minutes have to be subtracted from datetime.now() to match API server
...
def getlasttime():
	try:
		...
		print("appears like you haven't checked ever. How many minutes back in time do you want to go? ")
		minutes=int(input())
		if minutes<DELTATIME:
			minutes+=DELTATIME
		with open('lastpoll','wb') as lastpoll:
			lasttime=datetime.datetime.now()-datetime.timedelta(minutes=minutes)
			lasttimes=dict(zip(appslist,[dict(zip(["commits","issues","comments"],[lasttime]*3))]*len(appslist)))
...
branches=[[android,"dev"],[webogram,"master"],[cli,"master"],[tsupand,"dev"],[tdesk,"master"]]
